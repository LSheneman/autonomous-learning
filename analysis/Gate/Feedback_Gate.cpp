...
void FeedbackGate::update(vector<double> & states, vector<double> & nextStates) {
    ...
    if ((feedbackON) && (nrPos != 0) && (states[posFBNode] > 0.0)) {
        ...
        for (i = 0; i < chosenInPos.size(); i++) {  //posLevelOffFB[0..chosenInPos.size()]  once per lifetime!
...
double FeedbackGate::computeMutualInfo(){
    ...
    for(int i=0;i<normalizedTable.size();i++){
        ...
        for(int j=0;j<normalizedTable[i].size();j++){
...
double FeedbackGate::computeChannelEntropy(){
    double normalizationValue=0.0;
    vector<double>columns;
    vector<double> rows;
    double mutualInfo=0.0;
    
    rows.resize(entropyTable.size());
    columns.resize(entropyTable[0].size());
    // easier:
    for(int i=0;i<rows.size();i++)
        rows[i]=0.0;
    for(int j=0;j<columns.size();j++)
        columns[j]=0.0;
    //allows for est. of mutual information to be under est. --> easier to sell then over
    for (int i = 0; i < entropyTable.size(); i++)
        for (int j = 0; j < entropyTable[i].size(); j++){
            if(entropyTable[i][j]<=0.0){
                entropyTable[i][j]=0.01;
            }
            normalizationValue+=entropyTable[i][j];
        }
    
    //normalize the entropy table
    for (int i = 0; i < entropyTable.size(); i++){
        for (int j = 0; j < entropyTable[i].size(); j++){
            entropyTable[i][j]=entropyTable[i][j]/normalizationValue;
            columns[j]+=entropyTable[i][j];
            rows[i]+=entropyTable[i][j];
        }
    }
    ...
    for(int i=0;i<entropyTable.size();i++){
        for(int j=0;j<entropyTable[i].size();j++){
            if(entropyTable[i][j]>0.0){
                mutualInfo+=entropyTable[i][j]*log2(entropyTable[i][j]/(columns[j]*rows[i]));
            }
        }
    }
...
double FeedbackGate::getAppliedPosFeedback(){
    return totalPosForce;
}

double FeedbackGate::getAppliedNegFeedback(){
    return totalNegForce;
}