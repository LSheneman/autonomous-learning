...
double Organism::computeMutualInfo(){
    double totalMutalInfo=0.0;
    for(int i=0;i<brain->numGates();i++){
        totalMutalInfo+=brain->gates[i]->computeMutualInfo();
    }
    return totalMutalInfo;
}

double Organism::computeChannelEntropy(){
    double entropy=0.0;
    int counter=0;
    for(int i=0;i<brain->numGates();i++){
        if(brain->gates[i]->name()=="Feedback Gate"){
            counter+=1;
            entropy+=brain->gates[i]->computeChannelEntropy();
        }
    }
    return entropy;
}

double Organism::getAppliedPosFeedback(){
    int counter=0;
    double feedback=0.0;
    for(int i=0;i<brain->numGates();i++){
        if(brain->gates[i]->name()=="Feedback Gate"){
            counter+=1;
            feedback+=brain->gates[i]->getAppliedPosFeedback();        }
    }
    return feedback;
}

double Organism::getAppliedNegFeedback(){
    int counter=0;
    double feedback=0.0;
    for(int i=0;i<brain->numGates();i++){
        if(brain->gates[i]->name()=="Feedback Gate"){
            counter+=1;
            feedback+=brain->gates[i]->getAppliedNegFeedback();      }
    }
    return feedback;
}
