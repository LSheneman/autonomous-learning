...
string Organism::getAppliedPosFeedback(){
    int counter=0;
    string feedback="";
    for(int i=0;i<brain->numGates();i++){
        if(brain->gates[i]->name()=="Feedback Gate"){
            counter+=1;
            feedback+=" FeedbackGate"+ to_string(counter) +brain->gates[i]->getAppliedPosFeedback();        }
    }
    return feedback;
}

string Organism::getAppliedNegFeedback(){
    int counter=0;
    string feedback="";
    for(int i=0;i<brain->numGates();i++){
        if(brain->gates[i]->name()=="Feedback Gate"){
            counter+=1;
            feedback+=" FeedbackGate"+ to_string(counter) +brain->gates[i]->getAppliedNegFeedback();        }
    }
    return feedback;
}
