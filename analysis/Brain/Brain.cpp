...
vector<string> Brain::getStats() {
  ...
  dataPairs.push_back(to_string(gates.size()));
    ...
    int probilisticCount=0;
    for(auto gate:gates){
        if(gate->name()=="Feedback Gate"){
            feedbackCount+=1;
        }else if (gate->name()=="Probilistic Gate"){
            probilisticCount+=1;
        }else if (gate->name()=="Deterministic Gate"){
            deterministicCount+=1;
        }
    }
    dataPairs.push_back("feedbackCount");
    dataPairs.push_back(to_string(feedbackCount));
    dataPairs.push_back("probilisticCount");
    dataPairs.push_back(to_string(probilisticCount));
    dataPairs.push_back("deterministicCount");
    dataPairs.push_back(to_string(deterministicCount));
  return (dataPairs);
}
